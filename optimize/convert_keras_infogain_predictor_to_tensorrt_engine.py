...
def populate_network(network, weights):
    ...
    fc_state_feature_dense0_mult = network.add_matrix_multiply(rnn.get_output(0), trt.MatrixOperation.NONE, fc_state_feature_dense0_w.get_output(0), trt.MatrixOperation.NONE)
    ...
    fc_state_feature_reshape.reshape_dims = trt.Dims([ACTION_HORIZON, 32, 1, 1])
    ...
    add3_relu_reshape.reshape_dims = trt.Dims([ACTION_HORIZON, 8640])
...
def build_engine(weights):
    ...
    with trt.Builder(TRT_LOGGER) as builder, builder.create_network() as network, builder.create_builder_config() as config:
        ...
        config.max_workspace_size = GiB(2) #GiB(1)
