...
class MessageHandler():
    def __init__(self):
        self.autopilot_off = rospy.Publisher("/delta/autopilot/off", Empty,
                                             queue_size=1)
        self.arm_bridge = rospy.Publisher("/delta/bridge/arm", Bool,
                                          queue_size=1)
        self.autopilot_start = rospy.Publisher("/delta/autopilot/start", Empty,
                                               queue_size=1)
        self.autopilot_pose_cmd = rospy.Publisher("/delta/autopilot/pose_command",
                                                  PoseStamped, queue_size=1)
        self.tree_spacing_cmd = rospy.Publisher("/delta/tree_spacing",
                                                Float32, queue_size=1)
        self.obj_spacing_cmd = rospy.Publisher("/delta/object_spacing",
                                               Float32, queue_size=1)
        self.reset_exp_pub = rospy.Publisher("/success_reset",
                                             Empty, queue_size=1)
        self.save_pc_pub = rospy.Publisher("/delta/save_pc",
                                           Bool, queue_size=1)
...
def setup_sim(msg_handler, config):
    ...
    start_string = "rosservice call /gazebo/set_model_state " + \
     "'{model_state: { model_name: delta, pose: { position: { x: %f, y: %f ,z: %f }, " % (position[0],position[1],position[2]) + \
